...
public partial class AutocompletePlayerComponent
{
    [Parameter]
    ...
    private void OnHandleReadData(AutocompleteReadDataEventArgs autocompleteReadDataEventArgs)
    {
        ...
        {
            CurrentDataSource =
                Autocomplete.SearchAutocompletePlayer(AutocompleteData, autocompleteReadDataEventArgs.SearchValue);
        }
    }
...
#pragma warning restore CS4014
    ...
    private async Task Onkeypress(KeyboardEventArgs obj)
    {
        if (obj.Key is "Enter" or "NumpadEnter")
        {
            ...
            if (IsQuizPage)
            {
                ...
                await ClientState.Session!.hubConnection!.SendAsync("SendGuessChangedRigger", Guess);
            }
    ...
    {
        CurrentDataSource =
            Autocomplete.SearchAutocompletePlayer(AutocompleteData,
