...
public class ObjectToArrayConverter<T> : JsonConverter
{
        ...
        JsonSerializer serializer)
    {
        ...
        if (reader.TokenType != JsonToken.StartArray)
            throw new JsonSerializationException(string.Format("token {0} was not JsonToken.StartArray",
                reader.TokenType));
                ...
                switch (reader.ReadToContentAndAssert().TokenType)
                {
                    ...
                    default:
                        ...
                        if (property.Converter != null && property.Converter.CanRead)
                            propertyValue = property.Converter.ReadJson(reader, property.PropertyType,
                                property.ValueProvider.GetValue(existingValue), serializer);
                        else
